...
The left panel shows the achievable estimation--communication trade-off for \textbf{DP-trace} (sweeping $\lambda$), \textbf{ET} (sweeping the threshold $\delta$), and \textbf{PER} (sweeping the period $M$).
The right panel reports the estimation-cost gap $\Delta J_P := J_P^{\mathrm{heur}}-J_P^{\mathrm{DP}}$ under matched communication usage $J_C$, which makes the performance difference relative to the DP benchmark directly interpretable.

\begin{figure*}[t]
\centering
\includegraphics[width=\linewidth]{figs/fig_A_tradeoff_curves.pdf}
\caption{Finite-horizon trade-offs and benchmark gaps.
Left: estimation cost $J_P$ vs.\ communication usage $J_C$ for DP-trace, ET, and PER.
Right: estimation-cost gap $\Delta J_P$ relative to DP-trace under matched $J_C$ (lower is better).
}
\label{fig:tradeoff}
\end{figure*}
...
Fig.~\ref{fig:timeresp} shows a representative \textbf{DP-trace} rollout.
The policy is approximately a \emph{time-varying} trace threshold: it attempts transmission when $\mathrm{tr}(P_k)$ exceeds a time-dependent switching value $s_k^\star$ induced by the finite-horizon Bellman recursion.
Between successful receptions (ACKs), the covariance grows according to $\mathcal{T}(\cdot)$; a successful reception triggers an update $\mathcal{M}(\cdot)$ that ``resets'' uncertainty.
As the horizon approaches, $s_k^\star$ increases, reflecting the diminishing value of information when fewer future stages remain.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/fig_B_time_response.pdf}
\caption{Sample DP-trace rollout.
Top: covariance trace $\mathrm{tr}(P_k)$ and the induced time-varying DP switching threshold $s_k^\star$.
...
Left: the Lagrangian objective $J_\lambda = J_P + \lambda J_C$ under a fixed $\lambda$.
Right: the corresponding communication usage $J_C$.
Retuning the scheduler to the true $p$ reduces $J_\lambda$ relative to deploying a nominal policy under mismatch.
}
...
We report normalized performance relative to the nominal i.i.d.\ setting.
For the bursty case, we keep the mean success probability fixed and vary the temporal correlation (mean loss-burst length), isolating the impact of burstiness beyond the marginal drop rate.
While mismatch and burstiness can degrade performance, DP-based policies remain a useful finite-horizon reference for benchmarking heuristic schedulers.

\begin{figure*}[t]
\centering
\includegraphics[width=\linewidth]{figs/fig_D_robustness.pdf}
\caption{Robustness under channel mismatch (left) and bursty losses (right), shown as normalized performance relative to the nominal i.i.d.\ setting.
Right panel varies burstiness (mean loss-burst length) at fixed mean success probability.}
