...
Estimation cost $J_P$ versus communication usage $J_C$ for \textbf{DP-trace}, \textbf{ET}, and \textbf{PER}, with representative vertical segments indicating the gap to \textbf{DP-trace} at matched $J_C$ (lower is better).}
\label{fig:tradeoff}
\end{figure}
...
We organize the experiments around three questions: (i) what performance--communication trade-off is achievable over a finite horizon; (ii) what mechanism explains the DP benchmark's advantage; and (iii) how informative the benchmark remains under non-ideal channel conditions.
...
\end{equation}
which is \emph{not} optimized by~\eqref{eq:mdp_problem} but provides an interpretable measure of regulation quality under the fixed controller.
All DP-based policies are computed offline via the discretized backward-induction procedure in Section~\ref{sec:compute}.
All reported curves are Monte Carlo averages over independent realizations of the process/measurement noise and packet-drop processes, using common random seeds across methods for fair comparisons.
...
The horizon is $T=120$ steps.
Process and measurement noises are zero-mean Gaussian with standard deviations $(\sigma_w,\sigma_v)=(0.03,0.02)$, and packet delivery follows the i.i.d.\ Bernoulli model~\eqref{eq:drop} with nominal success probability $p=0.85$.
...
\end{itemize}
Unless otherwise stated, all methods are evaluated under identical noise statistics and identical channel parameters, and each curve is obtained by averaging over the same number of Monte Carlo rollouts.
For the optional counterexample in Section~\ref{sec:trace_not_enough}, we additionally evaluate \textbf{DP-2feature} from Section~\ref{sec:feature_disc}, which enriches the discretization with a shape-sensitive covariance feature.
...
Across the sweep, \textbf{DP-trace} forms the lower envelope of achievable trade-offs, reflecting explicit finite-horizon optimization over the covariance information state.
Both heuristics incur higher estimation cost at comparable communication usage, with the gap most pronounced in the intermediate regime where scheduling choices have the largest marginal value.

To make the comparison directly interpretable without an extra subplot, the vertical segments in Fig.~\ref{fig:tradeoff} annotate the estimation-cost gap
\begin{equation}
\Delta J_P := J_P^{\mathrm{heur}}-J_P^{\mathrm{DP}}
\end{equation}
under matched communication usage $J_C$.
This isolates allocation efficiency (when to transmit) from communication volume (how much to transmit), and highlights the finite-horizon, time-dependent structure captured by the DP benchmark.
...
Top: covariance trace $\mathrm{tr}(P_k)$ and the induced time-varying switching boundary $s_k^\star$, with transmission attempts and ACKs overlaid as rug marks.
...
The top panel plots the covariance trace $\mathrm{tr}(P_k)$ together with the induced time-varying DP switching boundary $s_k^\star$, and overlays transmission attempts and ACKs as rug marks.
The scheduling behavior is well-approximated by a \emph{time-dependent} threshold rule: a transmission is attempted when $\mathrm{tr}(P_k)$ exceeds $s_k^\star$.

Between successful receptions, the covariance grows according to the prediction map $\mathcal{T}(\cdot)$; a successful delivery triggers the Kalman update $\mathcal{M}(\cdot)$ and sharply reduces uncertainty, producing the characteristic grow-and-reset pattern.
The boundary $s_k^\star$ increases as the horizon approaches, reflecting the diminishing value of information with fewer remaining stages.
The bottom panel shows that ACK events coincide with drops in the estimation error norm $\|x_k-\hat{x}_k\|_2$.



\subsection{Channel Impairments}
\label{subsec:channel_impairments}

Finite-horizon schedules depend on the channel success probability $p$ through the transition model and, for DP policies, through the value-of-information term.
Fig.~\ref{fig:channel_impairments} summarizes how channel conditions affect sensitivity and robustness.
Panels (a)--(b) sweep the true $p$ and compare deploying a nominally designed policy (\emph{no-retune}) versus recomputing the scheduler for the true $p$ (\emph{retune}).
Retuning consistently improves the Lagrangian objective $J_\lambda := J_P + \lambda J_C$, and panel (b) quantifies the improvement as a percentage gain.

Panels (c)--(d) stress-test robustness beyond the nominal i.i.d.\ drop model.
Panel (c) reports normalized degradation under probability mismatch, for both estimation cost ($J_P$) and the closed-loop proxy ($J_X$), relative to the nominal design point.
Panel (d) evaluates temporally correlated (bursty) losses modeled by a Gilbert--Elliott channel at fixed mean success probability, with burstiness controlled by the mean loss-burst length.
Although mismatch and burstiness degrade performance, the DP benchmark remains informative as a strong finite-horizon reference against which structured heuristics can be evaluated.

\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{figs/fig_C_channel_impairments.pdf}
\caption{Channel impairments summary.
(a) Lagrangian objective $J_\lambda = J_P + \lambda J_C$ versus packet success probability $p$.
(b) Retune gain in $J_{\lambda}$, reported as a percentage improvement over no-retune, versus $p$.
(c) Normalized degradation under mismatch in the packet success probability, reported for both estimation cost ($J_P$) and regulation proxy ($J_X$), relative to the nominal i.i.d.\ setting.
(d) Performance under bursty packet drops (Gilbert--Elliott channel) at fixed mean success probability, as a function of burstiness (mean loss-burst length).}
...
The trace-only surrogate in Section~\ref{sec:trace_surrogate} is attractive for offline computation, but it can be strictly suboptimal because $\mathrm{tr}(P)$ discards directional information.
To illustrate when this loss of information matters, we consider a two-dimensional anisotropic system with partial observation
\[
...
\]
In this setting, covariances with the same trace can imply different future uncertainty growth due to anisotropy and sensing directions.
...
We compare \textbf{DP-trace} against \textbf{DP-2feature} from Section~\ref{sec:feature_disc}, which discretizes the covariance using both trace and a shape-sensitive feature (log-det).
\IfFileExists{figs/fig2_gap_curve.pdf}{
Fig.~\ref{fig:gapcurve} reports the estimation-cost gap under matched communication usage, demonstrating that incorporating even low-dimensional shape information can yield a nontrivial improvement in anisotropic settings.
