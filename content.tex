...
Fig.~\ref{fig:tradeoff} shows $J_P$ as a function of $J_C$.
...
To make the comparison directly interpretable, the vertical segments annotate the estimation-cost gap
\begin{equation}
...
Fig.~\ref{fig:channel_impairments} provides a consolidated view of how channel conditions affect both objective performance and robustness.
Panels (a)--(b) quantify sensitivity to $p$ by comparing: (i) \emph{no-retune} execution, where a policy designed for a nominal $p$ is deployed under a different true channel success probability; and (ii) \emph{retune} execution, where the policy is recomputed using the true $p$.
Retuning yields consistent improvements in $J_\lambda := J_P + \lambda J_C$ and changes communication usage, underscoring that $p$ influences not only reliability but also the optimal allocation of transmission attempts over a finite horizon.

Panels (c)--(d) report robustness stress tests.
In (c), we introduce mismatch in the assumed i.i.d.\ drop probability and report normalized degradation relative to the nominal design point, for both estimation performance ($J_P$) and the closed-loop proxy ($J_X$).
In (d), we consider temporally correlated (bursty) packet drops modeled by a Gilbert--Elliott channel.
...
(b) Communication-usage change $\Delta J_C$ between retune and no-retune versus $p$.
