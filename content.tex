...
\begin{figure*}[t]
\centering
\includegraphics[width=0.9\linewidth]{figs/fig_A_tradeoff_curves.pdf}
\caption{Finite-horizon trade-offs and benchmark gaps.
Left: estimation cost $J_P$ versus communication usage $J_C$ for \textbf{DP-trace}, \textbf{ET}, and \textbf{PER}.
Right: estimation-cost gap $\Delta J_P$ relative to the \textbf{DP-trace} benchmark under matched $J_C$ (lower is better).}
\label{fig:tradeoff}
\end{figure*}
...
The left panel of Fig.~\ref{fig:tradeoff} shows $J_P$ as a function of $J_C$.
...
To make the comparison directly interpretable, the right panel plots the estimation-cost gap
\begin{equation}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.9\linewidth]{figs/fig_C_sensitivity_p.pdf}
\caption{Sensitivity to packet success probability $p$.
Left: Lagrangian objective $J_\lambda = J_P + \lambda J_C$ (fixed $\lambda$).
Right: resulting communication usage $J_C$.
...
The left panel reports the Lagrangian objective $J_\lambda := J_P + \lambda J_C$ for a fixed $\lambda$, while the right panel reports the induced communication usage $J_C$.
...
In the left panel, we introduce mismatch in the assumed i.i.d.\ drop probability and report normalized degradation relative to the nominal design point, for both estimation performance ($J_P$) and the closed-loop proxy ($J_X$).
In the right panel, we consider temporally correlated (bursty) packet drops modeled by a Gilbert--Elliott channel.
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.9\linewidth]{figs/fig_D_robustness.pdf}
\caption{Robustness to non-ideal channels.
Left: normalized degradation under mismatch in the packet success probability, reported for both estimation cost ($J_P$) and regulation proxy ($J_X$), relative to the nominal i.i.d.\ setting.
Right: performance under bursty packet drops (Gilbert--Elliott channel) at fixed mean success probability, as a function of burstiness (mean loss-burst length).}
